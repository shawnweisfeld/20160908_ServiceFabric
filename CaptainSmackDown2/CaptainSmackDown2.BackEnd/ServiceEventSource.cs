...
{
    [EventSource(Name = "MyCompany-CaptainSmackDown2-BackEnd")]
    internal sealed class ServiceEventSource : EventSource
    {
        ...
        private const int ServiceInstanceConstructedEventId = 8;
        [Event(ServiceInstanceConstructedEventId, Level = EventLevel.Informational, Message = "Constructed instance of type {0}")]
        public void ServiceInstanceConstructed(string serviceType)
        {
            this.WriteEvent(ServiceInstanceConstructedEventId, serviceType);
        }

        private const int RunAsyncInvokedEventId = 9;
        [Event(RunAsyncInvokedEventId, Level = EventLevel.Informational, Message = "RunAsync invoked in service of type {0}")]
        public void RunAsyncInvoked(string serviceType)
        {
            this.WriteEvent(RunAsyncInvokedEventId, serviceType);
        }

        private const int CreateCommunicationListenerEventId = 10;
        [Event(CreateCommunicationListenerEventId, Level = EventLevel.Informational, Message = "Create communication listener in service instance of type {0}")]
        public void CreateCommunicationListener(string serviceType)
        {
            this.WriteEvent(CreateCommunicationListenerEventId, serviceType);
        }

        private const int RunAsyncStatusEventId = 11;
        [Event(RunAsyncStatusEventId, Level = EventLevel.Informational, Message = "{0} | Remaining Queued Items: {1}: <{2}, {3}>")]
        public void RunAsyncStatus(Guid partitionId, long queueLength, string captain, long count)
        {
            this.WriteEvent(RunAsyncStatusEventId, partitionId, queueLength, captain, count);
        }
